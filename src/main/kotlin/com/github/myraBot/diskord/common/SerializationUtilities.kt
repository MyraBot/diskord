...
val JSON_WITH_NULLS = Json {
    ignoreUnknownKeys = true
    encodeDefaults = true
    explicitNulls = true
}

inline fun <reified T> T.toJson(explicitNulls: Boolean = false): String {
    return if (explicitNulls) JSON_WITH_NULLS.encodeToString(this)
    else JSON.encodeToString(this)
}

inline fun <reified T> T.toJsonObj(explicitNulls: Boolean = false): JsonObject {
